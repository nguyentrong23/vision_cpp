...
namespace ImageMatch {
    GeoModel::GeoModel() {  // constructer defaul
        InitModelParameter();
    }

    GeoModel::GeoModel(string path, string modelName) {  // constructer para
        InitModelParameter();
        setImageOfModel(path);
        nameOfModel = modelName;
    }

    GeoModel::~GeoModel() {   // destructer

    }

    void GeoModel::InitModelParameter() {  //init
    ...
    // đọc ảnh và init vị trí gắp
    void GeoModel::setImageOfModel(string modelPath) {  // check path pattern  valid
        if (modelPath.empty()) {
            return;
        }
        ...
        if (imageOfMOdel.empty()) {   // check img
            return;
        }
        ...
        nameOfFile = modelPath.substr(splitIndex + 1, modelPath.length() - splitIndex);// mục đích cuối là trích xuất tên file
...
//        cvtColor(adjustContrast(imageOfMOdel, cannyContrast), imgGray, COLOR_RGB2GRAY);
        ...
        double sigma = (0.1 / (double)CANNY_RANGE_MAX) * cannyRange;  // cannyRange =50
        cannyThreshLower = std::max(0.0, ((1.0 - sigma) * medianValue));
        cannyThreshUpper = std::min(255.0, ((1.0 + sigma) * medianValue));
        Canny(imgGray, imgOutput, cannyThreshLower, cannyThreshUpper,cannyKernelSize);
        // find contours
        ...
        // save pattern info
        for (int contourIdx = 0; contourIdx < contoursOfModel.size(); contourIdx++) { // duyệt qua từng contours
            for (int pointInx = 0; pointInx < contoursOfModel[contourIdx].size(); pointInx++)  // từng điểm trên contours
            {
                ModelPattern tempData;
                double mag;
                tempData.Coordinates = contoursOfModel[contourIdx][pointInx];
                tempData.Derivative = Point2d(gx.at<double>(tempData.Coordinates),gy.at<double>(tempData.Coordinates));
                ...
                // push to container
                patternOfModel.push_back(tempData);  // vector từng model pattern
                sumPoint += tempData.Coordinates;  // tính tổng tọa độ để tính tọa độ tâm
            }
        }

        // compute template center point and update for all emlement in container
        Point2f templateCenterPoint(sumPoint.x / patternOfModel.size(),sumPoint.y / patternOfModel.size());
        ...
        }// update full phần  center vs offset
        ...
        int limitArea = ((double)(imgGray.cols * imgGray.rows) * 0.98);  // diện tích tối da
        int maxArea = 0;
        for (int conCounter = 0; conCounter < pcaContours.size(); conCounter++) {
            int area = contourArea(pcaContours[conCounter]); // tính diện tích của từng contours
            if (area >= limitArea) { // pass nếu countours bị nhiễu
                continue;
            }
            if (area > maxArea) {   // lấy diện tích  countours lớn nhất, index 
                maxArea = area;
                contoursSelectIndex = conCounter;
                contoursSelectArea = maxArea;
            }
        }
        computePcaOrientation(pcaContours[contoursSelectIndex], rawPcaAngle, centerOfPCA);// tính góc và hường của contours by PCA
    ...
    Mat GeoModel::getImageOfModel() {
        return imageOfMOdel.clone();   // tạo một bản sao
    }
    ...
    void GeoModel::drawPcaAxis(Mat& img, Point center, double angle, int length, double hookSize,Scalar xColour, Scalar yColour, Scalar centerColour)
    {
    ...
    Mat GeoModel::adjustContrast(cv::Mat image, double alpha) {  /// điều chỉnh độ tương phản _ alpha
