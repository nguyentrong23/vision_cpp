...
namespace ImageMatch {
    ...
    bool GeoMatch::addGeoMatchModel(String pathTemplate, String modelName) {  // trích xuất đặt trưng từ pattern
        GeoModel tempModel(pathTemplate, modelName);
        if (tempModel.isImageEmpty()) {
            return false;
        }
        tempModel.modelLearnPattern(); // mỗi điểm ảnh Edges của model  là một đối tượng tập hơp
        // một vector chứa các đối tượng là patternOfModel
        //tính rawPcaAngle, centerOfPCA của model
    ...
    void GeoMatch::removeMatchModel(int index) {
        ...
        ModelSrc.erase(ModelSrc.begin() + index);  // xóa phần tử ở vị trí index khỏi vector ModelSrc, 
        //tự động dồn các phần tử vị trí index ra trước
    }
    ...
    void GeoMatch::removeMatchModel(int startIndex, int endIndex) {
        ...
        ModelSrc.erase(ModelSrc.begin() + startIndex, ModelSrc.begin() + endIndex);// same
    }


    void GeoMatch::modifyMatchModelAt(int index, GeoModel model) {  // add or replace model ở vị trí index
    ...
    void GeoMatch::matching(bool boudingBoxChecking, int ObjectsNum) {
        ...
        imageSource.copyTo(matchingResult.Image); // Copy ảnh từ trigger vào cấu trúc MatchResult
        matchingResult.isFoundMatchObject = false;   // flag
        if (ModelSrc.empty()) {  // nếu pattern chưa có or erro pattern
        ...
        //imshow("srcContours", imageThresh);
        ...
        {  // duyệt qua từng contours
            PossibleObject tempObject;  // tạo một đối tượng có khả năng là vật cần gấp
            ...
            if (area <= minNoiseArea) {   // nếu diện tích contours nhiễu min 
                continue;
            }
            ...
            if (area >= maxNoiseArea) { // giải quyết vấn đề về dính chồng
                continue;
            }

            sumArea += area;  
            // check objects possible match with models in model source
            for (int modelCounter = 0; modelCounter < ModelSrc.size(); modelCounter++) { // duyệt qua từng model để đánh giá xem có model nào có diện tích gần bằng
                ...
                tempObject.modelCheckList.push_back(modelCounter);  // thêm cái index của model để check
            }

            if (tempObject.modelCheckList.size() > 0) { // object có tồn tại ít nhất 1 model cần check
                tempObject.conIndex = conCounter;  // thêm vào  in dẽ của contours
                ...
                GeoModel::computePcaOrientation(srcContours[conCounter], tempObject.pcaAngle, tempObject.pcaCenter);// tính pca cho contours
                objects.push_back(tempObject); // add vào vector cần check
            }
        }
        ...
        {  // duyệt qua só object trong vector
            double maxScores = 0.0;
            MatchedObjects matchObj;
            for (int modelCounter = 0; modelCounter < objects[objCounter].modelCheckList.size(); modelCounter++) { // duyệt qua từng model trong có cần check với contours
                int modelIndex = objects[objCounter].modelCheckList[modelCounter]; // lấy index model
                double rawAngle = ModelSrc[modelIndex].getModelRawPcaAngle();    // lấy góc pca
                Point2f ROICenter = objects[objCounter].pcaCenter;              // lấy tâm pca làm tâm ROI
                objects[objCounter].rotPositive.angle = objects[objCounter].pcaAngle + rawAngle;  // góc thuận 
                getRotatedROI(imageSource, objects[objCounter].rotPositive, ROICenter, objects[objCounter].conMinRectArea);// xoay ảnh theo góc thuận
                // update image crop, center và angle cho RotatedObject        (objects[objCounter].rotPositive)
                if (matchingScores(objects[objCounter].rotPositive, ModelSrc[modelIndex], maxScores)) { // match -> output: maxScores  vs center
                    saveMatchedObjectInfo(matchObj, objects[objCounter].rotPositive, ModelSrc[modelIndex], maxScores, objects[objCounter].pcaCenter);// lưu
                    break;
                }
            ...
            /// còn phần va chạm
            if (!matchObj.image.empty()) // nếu match được bất kì một model nào
            {
                //// CHECK PICKING BOX HAS INTERSECTION WITH ANOTHER BOUNDING BOX
                bool hasCollision = false;   //không có collision 
                // create object picking box
                matchObj.pickingBox = RotatedRect(matchObj.coordinates, pickingBoxSize, matchObj.angle * R2D);// có đủ không gian để robot gấp
                ...
                if (!hasCollision) {  /// chương trình k so sánh matchobject nào có matchscore cao hơn ???
        ...
        imshow("Rotated Image", matchingResult.Image);
        waitKey(0); // Wait for a key press
    }
...
//        matched.coordinates.y = pcaCenter.y + ((sin(objectRotated.angle)*xDist) + (cos(objectRotated.angle)*yDist));
    ...
    void GeoMatch::getRotatedROI(Mat& matSrc, RotatedObject& object, Point center, RotatedRect minRectArea) {
        ...
        // output của  getRotationMatrix2D [ cos(theta) -sin(theta) tx ]
        ...
        Rect BoundingRect = boundingRect(rotatedRect); // tọa độ 4 đỉnh của ảnh gốc sau khi xoay thành hình chữ nhật giới hạn
        // resize matrix to wrap all image
        rotationMatrix.at<double>(0, 2) = -BoundingRect.x;
        rotationMatrix.at<double>(1, 2) = -BoundingRect.y;
        // rotated image // xoay src bỏ vào boundingrect
        ...
        minRectArea.points(transformBoundingBox); // chuyển các đỉnh thành vào vector điểm transformBoundingBox
        transform(transformBoundingBox, transformBoundingBox, rotationMatrix);  // xoay cái rectarea của object theo rotationMatrix
        BoundingRect = boundingRect(transformBoundingBox);

        object.image = cropImageWithBorderOffset(object.image, BoundingRect, 2); // crop box chỉ còn object tương ứng

        // get center off PCA for transform pickposition
        vector<Point2f> transformCenterPoint;
        transformCenterPoint.push_back(center);
        transform(transformCenterPoint, transformCenterPoint, rotationMatrix);// đưa tâm pca đi xoay
        Point2f topleft = BoundingRect.tl();        // top left
        object.centerCrop = transformCenterPoint[0] - topleft; // tính tâm mới từ tâm cũ trừ topleft (vi crop)
    }
    ...
    bool GeoMatch::matchingScores(RotatedObject& objectRotated, GeoModel& model, double& lastMaxScores) {
        ...
        vector<GeoModel::ModelPattern> modelPattern = model.getPatternOfModel();// gáng vector model

        // ncc match search
        long noOfCordinates = modelPattern.size();     // số model
        ...
        Point2f offset = model.getDistanceCenterPattern(); // khoảng cách từ tâm object tới tâm pca
